<template>
  <div>
    ...
    <div class="flex flex-col m-auto w-64">
      <input
        class="my-4 bg-grey-200 apperance-none border-2 border-grey-200 rounded w-full py-2 px-4 text-grey-700 leading-tight focus:outline-none focus: bg-white focus:border-purple-500"
...
export default {
  ...
  methods: {
    ...
    async createAlbum() {
      this.error = "";
      if (!this.albumName) {
        this.error = "Please enter album name";
        return;
      }
      const newAlbum = {
        name: this.albumName,
